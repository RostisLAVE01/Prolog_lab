...
fun main(){
    ...
    val drivers = listOf(Taxi, Personal, Truck, Truck1, Taxi1, Personal2,Truck2,Truck3,Taxi2)
...
fun getdriver_license(driver:List<Driver>):String{
    val rights = driver.maxByOrNull { it.licenseType }
    if (rights != null) {
        val rights_class = "Категория водительских прав: ${rights.licenseType}, Имя водителя: ${rights.name}, Возраст: ${rights.age}"
        return (rights_class)
    } else {
...
fun getSenior_employee(driver:List<Driver>):String{
    // Старший сотрудник
    val old = driver.maxByOrNull { it.age }
    if (old != null) {
        val old_drive = "Возраст: ${old.age}, Имя водителя: ${old.name}, Категория водительских прав: ${old.licenseType}"
        return (old_drive)
    } else {
...
fun Percentage_with_guard_for_more_than_5_years(driver: List<Driver>):String{
    // Процент со стражем больше 5 лет
    val experience = (driver.count { it.hireYear > 5 }.toDouble() / driver.size) * 100
    val intermediate1 = "%.1f".format(experience)
    val Percent1 = "Процент со стражем больше 5 лет: $intermediate1%"
    return (Percent1)
}

fun driving_license_category_B_and_over_25(driver: List<Driver>):String{
    // Процент людей с категорией прав B и старше 25
    val category_of_rights = driver.filter { it.licenseType == "B" }
    val intermediate2 = (category_of_rights.count { it.age > 25 }.toDouble() / category_of_rights.size) * 100
    val Percent2 = "Процент людей с категорией прав B и старше 25: $intermediate2%"
    return (Percent2)
}

fun Average_work_experience(driver: List<Driver>):String{
    // Средний стаж работы
    val Average_length_of_service = driver.filter { it.licenseType == "D" }.map {it.hireYear}.average()
    val Percent3 = "Средний стаж работы: $Average_length_of_service"
    return (Percent3)
}
